    ...
    for path in join_paths {
        ...
        for (i, segment) in segments.iter().enumerate() {
            ...
            let relation = entity_def.relations.get(*segment).ok_or_else(|| {
                AppError::BadRequest(format!("Unknown relation: {}.{}", current_entity, segment))
            })?;
...
pub fn resolve_field(
    ...
    let entity_def = SCHEMA
        .get(entity_name)
        .ok_or_else(|| AppError::BadRequest(format!("Unknown entity in field: {}", entity_name)))?;

    let field_def: &FieldDef = entity_def.fields.get(field_name).ok_or_else(|| {
        AppError::BadRequest(format!("Field not allowed: {}.{}", entity_name, field_name))
    })?;

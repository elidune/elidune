...
use super::schema::{FieldDef, FieldKind, SCHEMA};
...
impl ResolvedField {
    ...
    pub fn sql_expr(&self) -> String {
        match self {
            ResolvedField::Physical { table_alias, column } => format!(r#""{}"."{}""#, table_alias, column),
...
pub fn resolve_joins(root_entity: &str, join_paths: &[String]) -> Result<AliasMap, AppError> {
    SCHEMA.get(root_entity).ok_or_else(|| AppError::BadRequest(format!("Unknown root entity: {}", root_entity)))?;
    ...
    for path in join_paths {
        ...
        for (i, segment) in segments.iter().enumerate() {
            ...
            let entity_def = SCHEMA
                .get(current_entity.as_str())
                .ok_or_else(|| AppError::BadRequest(format!("Unknown entity in join graph: {}", current_entity)))?;

            let relation = entity_def
                .relations
                .get(*segment)
                .ok_or_else(|| AppError::BadRequest(format!("Unknown relation: {}.{}", current_entity, segment)))?;

            let target_def = SCHEMA
                .get(relation.target_entity)
                .ok_or_else(|| AppError::BadRequest(format!("Relation target entity not registered: {}", relation.target_entity)))?;
...
pub fn resolve_field(field_path: &str, root_entity: &str, alias_map: &AliasMap) -> Result<ResolvedField, AppError> {
    let parts: Vec<&str> = field_path.splitn(2, '.').collect();
    let (entity_name, field_name) = if parts.len() == 2 { (parts[0], parts[1]) } else { (root_entity, parts[0]) };

    let entity_def = SCHEMA.get(entity_name).ok_or_else(|| AppError::BadRequest(format!("Unknown entity in field: {}", entity_name)))?;

    let field_def: &FieldDef = entity_def
        .fields
        .get(field_name)
        .ok_or_else(|| AppError::BadRequest(format!("Field not allowed: {}.{}", entity_name, field_name)))?;

    let table_alias = if let Some(node) = alias_map.get(entity_name) {
        node.alias.clone()
    } else {
        let candidates: Vec<&JoinNode> = alias_map.values().filter(|n| n.entity_name == entity_name).collect();

        match candidates.len() {
            0 => {
                return Err(AppError::BadRequest(format!("Entity '{}' is not joined; add it to joins", entity_name)));
            }
            1 => candidates[0].alias.clone(),
            _ => {
                return Err(AppError::BadRequest(format!("Ambiguous entity '{}'; qualify joins so only one path exists", entity_name)));
            }
        }
    };

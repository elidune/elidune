...
use crate::{
    ...
    models::dto::stats::{CatalogStatsResponse, Interval, LoanStatsResponse, StatsResponse, UserLoanStats, UserStatsAggregate, UserStatsSortBy},
...
impl StatsService {
    ...
    pub async fn get_user_stats(&self, sort_by: UserStatsSortBy, limit: i64) -> AppResult<Vec<UserLoanStats>> {
    ...
    pub async fn get_loan_stats(
        ...
        self.repository.stats_get_loan_stats(start_date, end_date, interval, media_type, public_type, user_id).await
    }

    pub async fn get_user_aggregates(&self, start_date: Option<DateTime<Utc>>, end_date: Option<DateTime<Utc>>) -> AppResult<UserStatsAggregate> {
        self.repository.stats_get_user_aggregates(start_date, end_date).await
    }
    ...
    pub async fn get_catalog_stats(
        ...
        self.repository.stats_get_catalog_stats(start_date, end_date, by_source, by_media_type, by_public_type).await
    }
    ...
    pub async fn list_saved_queries(&self, user_id: i64, is_admin: bool) -> AppResult<Vec<SavedStatsQuery>> {
        crate::repository::stats::saved_queries::list_for_user(self.repository.pool(), user_id, is_admin).await
    }

    pub async fn create_saved_query(&self, user_id: i64, body: &SavedStatsQueryWrite) -> AppResult<SavedStatsQuery> {
        crate::repository::stats::saved_queries::insert(self.repository.pool(), user_id, body).await
    }

    pub async fn update_saved_query(&self, id: i64, user_id: i64, is_admin: bool, body: &SavedStatsQueryWrite) -> AppResult<SavedStatsQuery> {
        crate::repository::stats::saved_queries::update(self.repository.pool(), id, user_id, is_admin, body).await
    }

    pub async fn delete_saved_query(&self, id: i64, user_id: i64, is_admin: bool) -> AppResult<()> {
        crate::repository::stats::saved_queries::delete_by_id(self.repository.pool(), id, user_id, is_admin).await
    }

    pub async fn run_saved_query(&self, id: i64, user_id: i64, is_admin: bool) -> AppResult<StatsTableResponse> {
        let saved = crate::repository::stats::saved_queries::get_by_id(self.repository.pool(), id, user_id, is_admin)
            .await?

...
const WARN_EMPTY_SCOPE: &str = "No active copies match the selected scope (source and/or place); verify catalog data before scanning.";
        ...
        for (index, item_id) in missing_ids.into_iter().enumerate() {
            match self.catalog.delete_item(item_id, force).await {
                ...
                Err(AppError::Conflict(reason)) if !force => {
                    skipped.push(InventoryConsolidationSkipped { item_id, reason });
                }
                Err(e) => return Err(e),
            }

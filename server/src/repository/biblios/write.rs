...
impl Repository {
    ...
    pub async fn biblios_delete(&self, id: i64, force: bool) -> AppResult<()> {
        ...
        sqlx::query("UPDATE items SET archived_at = $1, updated_at = $1, barcode = CONCAT('ARCH_', id::text, '_', COALESCE(barcode, '')) WHERE biblio_id = $2 AND archived_at IS NULL")
            .bind(now)
            .bind(id)
            .execute(&self.pool)
            .await?;

        sqlx::query("UPDATE biblios SET archived_at = $1, updated_at = $1 WHERE id = $2")
            .bind(now)
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    async fn process_collection(&self, collection: &Option<Collection>) -> AppResult<Option<i64>> {
        ...
        if let Some(id) = existing {
            ...
            let id = sqlx::query_scalar::<_, i64>("INSERT INTO collections (key, name, secondary_title, tertiary_title, issn) VALUES ($1, $2, $3, $4, $5) RETURNING id")
                .bind(&key)
                .bind(name)
    ...
    async fn process_edition(&self, edition: &Option<Edition>) -> AppResult<Option<i64>> {
        ...
        if let Some(id) = existing {
            ...
            let id = sqlx::query_scalar::<_, i64>("INSERT INTO editions (publisher_name, place_of_publication, date) VALUES ($1, $2, $3) RETURNING id")
                .bind(publisher_name)

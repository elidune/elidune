...
impl VisitorCountsRepository for super::Repository {
    ...
    ) -> crate::error::AppResult<Vec<crate::models::visitor_count::VisitorCount>> {
    ...
    async fn visitor_counts_create(
        &self,
        data: &crate::models::visitor_count::CreateVisitorCount,
    ) -> crate::error::AppResult<crate::models::visitor_count::VisitorCount> {
...
impl Repository {
    ...
    #[tracing::instrument(skip(self), err)]
    pub async fn visitor_counts_create(
        &self,
        data: &CreateVisitorCount,
    ) -> AppResult<VisitorCount> {
        let count_date = NaiveDate::parse_from_str(&data.count_date, "%Y-%m-%d").map_err(|_| {
            crate::error::AppError::Validation("Invalid count_date format".to_string())
        })?;
    ...
    pub async fn visitor_counts_delete(&self, id: i64) -> AppResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(crate::error::AppError::NotFound(format!(
                "Visitor count with id {} not found",
                id
            )));
        }
        Ok(())

...
use crate::{
    error::AppResult,
    models::loan::{CreateLoan, Loan, LoanCreateOutcome, LoanDetails, LoanMarcExportRow, LoanReturnOutcome, LoanSettings, LoanSettingsRenewAt},
};
...
pub trait LoansRepository: Send + Sync {
    ...
    async fn loans_get_borrower_for_loan(&self, loan_id: i64) -> AppResult<crate::models::user::UserShort>;
    async fn loans_get_by_item_identification(&self, item_identification: &str) -> AppResult<Loan>;
    async fn loans_get_for_user(&self, user_id: i64, page: i64, per_page: i64) -> AppResult<(Vec<LoanDetails>, i64)>;
    async fn loans_archives_get_for_user(&self, user_id: i64, page: i64, per_page: i64) -> AppResult<(Vec<LoanDetails>, i64)>;
    /// All loans for MARC export (no pagination). Active or archived only.
    async fn loans_get_for_marc_export(&self, user_id: i64, archived: bool) -> AppResult<Vec<LoanMarcExportRow>>;
    ...
    async fn loans_get_overdue_for_reminders(&self, frequency_days: u32) -> AppResult<Vec<OverdueLoanRow>>;
    async fn loans_get_overdue(&self, page: i64, per_page: i64) -> AppResult<(Vec<OverdueLoanRow>, i64)>;
    ...
    async fn loans_settings_upsert_row(&self, media_type: Option<String>, nb_max: i16, nb_renews: i16, duration: i16, renew_at: LoanSettingsRenewAt) -> AppResult<()>;
...
impl LoansRepository for Repository {
    ...
    async fn loans_get_borrower_for_loan(&self, loan_id: i64) -> crate::error::AppResult<crate::models::user::UserShort> {
    ...
    async fn loans_get_for_user(&self, user_id: i64, page: i64, per_page: i64) -> crate::error::AppResult<(Vec<LoanDetails>, i64)> {
        Repository::loans_get_for_user(self, user_id, page, per_page).await
    }
    async fn loans_archives_get_for_user(&self, user_id: i64, page: i64, per_page: i64) -> crate::error::AppResult<(Vec<LoanDetails>, i64)> {
        Repository::loans_archives_get_for_user(self, user_id, page, per_page).await
    }
    async fn loans_get_for_marc_export(&self, user_id: i64, archived: bool) -> crate::error::AppResult<Vec<LoanMarcExportRow>> {
    ...
    async fn loans_settings_upsert_row(&self, media_type: Option<String>, nb_max: i16, nb_renews: i16, duration: i16, renew_at: LoanSettingsRenewAt) -> crate::error::AppResult<()> {
        Repository::loans_settings_upsert_row(self, media_type, nb_max, nb_renews, duration, renew_at).await
    }

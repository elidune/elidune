...
impl Repository {
    ...
    pub async fn collections_list(
        ...
        let (rows, total) = if let Some(ref name) = query.name {
            let pattern = format!("%{}%", name.replace('%', "\\%").replace('_', "\\_"));
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM collections WHERE unaccent(lower(name)) LIKE unaccent(lower($1))")
                .bind(&pattern)
                .fetch_one(&self.pool)
                .await?;
    ...
    pub async fn collections_get(&self, id: i64) -> AppResult<Collection> {
        sqlx::query_as("SELECT id, key, name, secondary_title, tertiary_title, issn, created_at, updated_at FROM collections WHERE id = $1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await?

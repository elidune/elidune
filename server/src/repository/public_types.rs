...
impl PublicTypesRepository for super::Repository {
    async fn public_types_list(
        &self,
    ) -> crate::error::AppResult<Vec<crate::models::public_type::PublicType>> {
        super::Repository::public_types_list(self).await
    }
    async fn public_types_get_by_id(
        &self,
        id: i64,
    ) -> crate::error::AppResult<crate::models::public_type::PublicType> {
    ...
    async fn public_types_create(
        &self,
        data: &crate::models::public_type::CreatePublicType,
    ) -> crate::error::AppResult<crate::models::public_type::PublicType> {
    ...
    async fn public_types_update(
        &self,
        id: i64,
        data: &crate::models::public_type::UpdatePublicType,
    ) -> crate::error::AppResult<crate::models::public_type::PublicType> {
    ...
    async fn public_types_find_id_by_name(
        &self,
        name: &str,
...
impl Repository {
    ...
    pub async fn public_types_list(&self) -> AppResult<Vec<PublicType>> {
        Ok(
            sqlx::query_as::<_, PublicType>("SELECT * FROM public_types ORDER BY name")
                .fetch_all(&self.pool)
                .await?,
        )
    }
    ...
    #[tracing::instrument(skip(self), err)]
    pub async fn public_types_get_loan_settings(
        &self,
        public_type_id: i64,
    ) -> AppResult<Vec<PublicTypeLoanSettings>> {
        Ok(
    ...
    pub async fn public_types_create(&self, data: &CreatePublicType) -> AppResult<PublicType> {
        let public_type = sqlx::query_as::<_, PublicType>(
            ...
            INSERT INTO public_types (
                ...
                subscription_price, unpaid_fine_threshold, max_active_holds
            )
    ...
    #[tracing::instrument(skip(self), err)]
    pub async fn public_types_update(
        &self,
        id: i64,
        ...
        let subscription_duration_days = data
            .subscription_duration_days
            .or(existing.subscription_duration_days);
        ...
        let max_active_holds = data.max_active_holds.or(existing.max_active_holds);
        ...
        sqlx::query_as::<_, PublicType>(
            ...
            UPDATE public_types SET
                ...
                unpaid_fine_threshold = $7, max_active_holds = $8
            WHERE id = $9
            RETURNING *
            "#,
        )
    ...
    pub async fn public_types_delete(&self, id: i64) -> AppResult<()> {
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM users WHERE public_type = $1")
            .bind(id)
            .fetch_one(&self.pool)
            .await?;
    ...
    pub async fn public_types_find_id_by_name(&self, name: &str) -> AppResult<Option<i64>> {
        Ok(
            sqlx::query_scalar::<_, i64>("SELECT id FROM public_types WHERE name = $1")
                .bind(name)
                .fetch_optional(&self.pool)
                .await?,
        )

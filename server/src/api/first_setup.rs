...
impl FirstSetupEmailBody {
    fn into_email_config(self, file_defaults: &EmailConfig) -> EmailConfig {
        EmailConfig {
            ...
            templates_dir: self.templates_dir.unwrap_or_else(|| file_defaults.templates_dir.clone()),
...
pub async fn post_first_setup(State(state): State<AppState>, ClientIp(ip): ClientIp, ValidatedJson(body): ValidatedJson<FirstSetupRequest>) -> AppResult<(StatusCode, Json<FirstSetupResponse>)> {
    let repo = state.services.minimal_repository();
    if repo.users_count().await? != 0 || repo.settings_count().await? != 0 {
        return Err(AppError::Conflict("Initial setup has already been completed".into()));
    }

    let default_public_type: i64 = repo
        .public_types_first_id()
        .await?
        .ok_or_else(|| AppError::Internal("No public_type row found; run database migrations first".into()))?;
    ...
    let addr_city = body
        ...
        .ok_or_else(|| AppError::Validation("library.addrCity or library.name is required for the patron record".into()))?;

    if body.email.is_some() && !state.dynamic_config.is_overridable("email") {
        return Err(AppError::BadRequest("email.overridable must be true in the server config file to store SMTP from first setup".into()));
    }
    ...
    state.services.users.set_must_change_password(created.id, false).await?;

    let user = if let Some(lang) = admin.language {
        repo.users_update_profile(created.id, &profile_language_only(lang), None).await?
    } else {
        state.services.users.get_by_id(created.id).await?
    };

    let has_email_override = body.email.is_some();
    let library_info = state.services.library_info.update(body.library.clone()).await?;

    if let Some(email_body) = body.email {
        let merged = email_body.into_email_config(&state.config.email);
        let value = serde_json::to_value(&merged).map_err(|e| AppError::Internal(format!("serialize email config: {e}")))?;
        state.dynamic_config.update_section("email", value.clone())?;
        repo.settings_upsert_section("email", &value)
            .await

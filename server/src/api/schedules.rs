...
pub async fn create_period(
    ...
    state.services.audit.log(
        audit::event::SCHEDULE_PERIOD_CREATED,
        Some(claims.user_id),
        Some("schedule_period"),
        Some(period.id),
        ip,
        Some((&data, &period)),
...
pub async fn update_period(
    ...
    state.services.audit.log(
        audit::event::SCHEDULE_PERIOD_UPDATED,
        Some(claims.user_id),
        Some("schedule_period"),
        Some(id),
        ip,
...
pub async fn delete_period(State(state): State<crate::AppState>, AuthenticatedUser(claims): AuthenticatedUser, ClientIp(ip): ClientIp, Path(id): Path<i64>) -> AppResult<StatusCode> {
    ...
    state.services.audit.log(
        audit::event::SCHEDULE_PERIOD_DELETED,
        Some(claims.user_id),
        Some("schedule_period"),
        Some(id),
        ip,
...
pub async fn delete_slot(State(state): State<crate::AppState>, AuthenticatedUser(claims): AuthenticatedUser, ClientIp(ip): ClientIp, Path(id): Path<i64>) -> AppResult<StatusCode> {
    ...
    state.services.audit.log(
        audit::event::SCHEDULE_SLOT_DELETED,
        Some(claims.user_id),
        Some("schedule_slot"),
        Some(id),
        ip,
...
pub async fn create_closure(
    ...
    state.services.audit.log(
        audit::event::SCHEDULE_CLOSURE_CREATED,
        Some(claims.user_id),
        Some("schedule_closure"),
        Some(closure.id),
        ip,
        Some((&data, &closure)),
        audit::AuditLogMeta::success(),
...
pub async fn delete_closure(State(state): State<crate::AppState>, AuthenticatedUser(claims): AuthenticatedUser, ClientIp(ip): ClientIp, Path(id): Path<i64>) -> AppResult<StatusCode> {
    ...
    state.services.audit.log(
        audit::event::SCHEDULE_CLOSURE_DELETED,
        Some(claims.user_id),
        Some("schedule_closure"),
        Some(id),
        ip,

...
async fn overdue_reminder_enqueue_reserves_loans_until_sent() {
    ...
    let reminder_count: i32 = sqlx::query_scalar("SELECT reminder_count FROM loans WHERE id = $1")
        .bind(loan_ids[0])
        .fetch_one(repo.pool())
        .await
...
async fn link_reminder_loans(repo: &Repository, outbox_id: i64, loan_ids: &[i64]) {
    for loan_id in loan_ids {
        sqlx::query("INSERT INTO email_outbox_reminder_loans (outbox_id, loan_id) VALUES ($1, $2)")
            .bind(outbox_id)
            .bind(loan_id)
            .execute(repo.pool())
            .await

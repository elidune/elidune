...
| `GET /biblios/:id/holds` | JWT + `require_read_holds_staff()` | Title queue (copy-level then title-level); not allowed for **`own`**. |
...
| `POST /loans/:id/fines/accrue` | Staff |
| `POST /users/:id/fines/accrue` | Staff |
| `POST /fines/accrue` | Staff |
